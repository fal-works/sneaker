...
class LogTypeExtension {
	 ...
	 */
	public static inline function forcePrint(thisType:LogType, message:String, ?tag:Tag, ?pos:PosInfos):Void {
	 ...
	 */
	public static inline function printIfMatch(thisType:LogType, message:String, ?tag:Tag, ?pos:PosInfos):Void {
		if (!thisType.tagFilter(tag))
			return;

		if (!thisType.positionFilter(pos))
			return;
