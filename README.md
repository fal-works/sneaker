...
|flag|description|
|---|---|
|`sneaker_log_level`|Threshold for filtering logs by levels. Defaults to `200` (if `--debug`, `500`).|
|`sneaker_macro_log_level`|Threshold for filtering macro logs by levels. Defaults to `300` (if `--debug`, `500`).|
|`sneaker_macro_message_level`|Threshold for filtering messages to the completion server. Defaults to `300` (if `--debug`, `500`).|
